...
from  model_zoo.inflate_from_2d_model import inflate_from_2d_model
...
class S3D(nn.Module):
    ...
    def __init__(self, num_classes=1000, dropout_ratio=0.2, without_t_stride=False,
                 pooling_method='max', dw_t_conv=False, tam_pos='all'):
        super(S3D, self).__init__()
        self.pooling_method = pooling_method.lower()
        self.tam_pos = tam_pos
        ...
        if self.tam_pos == 'first':
            use_s3d_1 = True
            use_s3d_2 = False
        elif self.tam_pos == 'last':
            use_s3d_1 = False
            use_s3d_2 = True
        else:
            use_s3d_1 = True
        ...
        # TODO: half is hard-coded

        self.inception3a = Inception(192, 64, 96, 128, 16, 32, 32, dw_t_conv, use_s3d=use_s3d_1 if self.tam_pos != 'half' else True)
        self.inception3b = Inception(256, 128, 128, 192, 32, 96, 64, dw_t_conv, use_s3d=use_s3d_1 if self.tam_pos != 'half' else False)

        self.inception4a = Inception(480, 192, 96, 208, 16, 48, 64, dw_t_conv, use_s3d=use_s3d_1 if self.tam_pos != 'half' else True)
        self.inception4b = Inception(512, 160, 112, 224, 24, 64, 64, dw_t_conv, use_s3d=use_s3d_1 if self.tam_pos != 'half' else False)
        self.inception4c = Inception(512, 128, 128, 256, 24, 64, 64, dw_t_conv, use_s3d=use_s3d_1 if self.tam_pos != 'half' else True)
        self.inception4d = Inception(512, 112, 144, 288, 32, 64, 64, dw_t_conv, use_s3d=use_s3d_2 if self.tam_pos != 'half' else False)
        self.inception4e = Inception(528, 256, 160, 320, 32, 128, 128, dw_t_conv, use_s3d=use_s3d_2 if self.tam_pos != 'half' else True)

        self.inception5a = Inception(832, 256, 160, 320, 32, 128, 128, dw_t_conv, use_s3d=use_s3d_2 if self.tam_pos != 'half' else True)
        self.inception5b = Inception(832, 384, 192, 384, 48, 128, 128, dw_t_conv, use_s3d=use_s3d_2 if self.tam_pos != 'half' else False)
...
class Inception(nn.Module):
    ...
    def __init__(self, in_channels, ch1x1, ch3x3red, ch3x3, ch5x5red, ch5x5, pool_proj,
        ...
        self.branch2 = nn.Sequential(
            ...
            if use_s3d else BasicConv3d(ch3x3red, ch3x3, kernel_size=(1, 3, 3), padding=(0, 1, 1))
        )
        ...
        self.branch3 = nn.Sequential(
            ...
            if use_s3d else BasicConv3d(ch5x5red, ch5x5, kernel_size=(1, 3, 3), padding=(0, 1, 1))
        )
...
def s3d(num_classes, dropout, without_t_stride, pooling_method, dw_t_conv, tam_pos, **kwargs):
    model = S3D(num_classes=num_classes, dropout_ratio=dropout, without_t_stride=without_t_stride,
                pooling_method=pooling_method, dw_t_conv=dw_t_conv, tam_pos=tam_pos)
...
if __name__ == '__main__':
    ...
    model = s3d(400, 0.5, without_t_stride=False, pooling_method='max', dw_t_conv=False,
                tam_pos='half')
